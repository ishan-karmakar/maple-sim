...
DriveTrainSimulationConfig& DriveTrainSimulationConfig::WithTrackLengthTrackWidth(units::meter_t trackLengthX, units::meter_t trackWidthY) {
    ...
    moduleTranslations = {
        frc::Translation2d{trackLengthX / 2, trackWidthY / 2},
        frc::Translation2d{trackLengthX / 2, -trackWidthY / 2},
        frc::Translation2d{-trackLengthX / 2, trackWidthY / 2},
        frc::Translation2d{-trackLengthX / 2, -trackWidthY / 2}
    };
...
DriveTrainSimulationConfig& DriveTrainSimulationConfig::WithSwerveModules(std::initializer_list<std::function<SwerveModuleSimulation()>> swerveModuleSimulationFactory) {
    if (swerveModuleSimulationFactory.size() == 1) return WithSwerveModule(*swerveModuleSimulationFactory.begin());

    FRC_AssertMessage(swerveModuleSimulationFactory.size() == moduleTranslations.size(), "Module simulation factories length must be 1 or 4, provided {}", swerveModuleSimulationFactory.size());

    swerveModuleSimulationFactories = swerveModuleSimulationFactory;
    return *this;
}

DriveTrainSimulationConfig& DriveTrainSimulationConfig::WithSwerveModule(std::function<SwerveModuleSimulation()> swerveModuleSimulationFactory) {
...
void DriveTrainSimulationConfig::CheckModuleTranslations() const {
    for (int i = 0; i < moduleTranslations.size(); i++)
        BoundingCheck::Check(moduleTranslations[i].Norm(), 0.2_m, 1.2_m, "module number " + std::to_string(i) + " translation magnitude", "meters");
}
