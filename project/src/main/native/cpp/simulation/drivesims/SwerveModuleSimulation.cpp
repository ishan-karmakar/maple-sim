...
Eigen::Vector2d SwerveModuleSimulation::GetPropellingForce(units::newton_t grippingForce, frc::Rotation2d moduleWorldFacing, Eigen::Vector2d moduleCurrentGroundVelocity) {
    ...
    if (skidding) propellingForce = units::math::copysign(grippingForce, propellingForce);

    units::meters_per_second_t floorVelocityProjectionOnWheelDirection{moduleCurrentGroundVelocity.dot(moduleWorldFacing.ToMatrix().col(0))};

    driveWheelFinalSpeed = floorVelocityProjectionOnWheelDirection / config.WHEEL_RADIUS;

    if (skidding) {
        units::radians_per_second_t skiddingEquilibriumWheelSpeed = config.driveMotorConfigs.CalculateMechanismVelocity(
            config.driveMotorConfigs.CalculateCurrent(propellingForce * config.WHEEL_RADIUS),
            driveMotorAppliedVoltage
        );
        driveWheelFinalSpeed = driveWheelFinalSpeed / 2 + skiddingEquilibriumWheelSpeed / 2;
    }

    return Eigen::Vector2d{propellingForce * units::math::cos(moduleWorldFacing), propellingForce * units::math::sin(moduleWorldFacing)};
}

units::newton_meter_t SwerveModuleSimulation::GetDriveWheelTorque() {
    driveMotorAppliedVoltage = driveMotorController->UpdateControlSignal(driveWheelFinalPosition, driveWheelFinalSpeed, GetDriveEncoderUnGearedPosition(), GetDriveEncoderUnGearedSpeed());
    ...
    units::newton_meter_t driveWheelTorqueWithFriction = frc::ApplyDeadband(driveWheelTorque, config.driveMotorConfigs.friction, units::newton_meter_t{std::numeric_limits<double>::infinity()});
